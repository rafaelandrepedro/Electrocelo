...
# 10 "controlCurrent_MC50.c" 2

# 1 "./controlCurrent.h" 1
# 53 "./controlCurrent.h"
void ReadADC(void);
...
unsigned int ADC_array [30];
volatile unsigned int SumEnergyADCHist_array [10];
volatile unsigned int ADC_zero_array[30];
...
unsigned int sumEnergyADC(void)
{
    ...
    for ( i=0 ; i<(30) ; i++ )
    {
# 83 "controlCurrent_MC50.c"
        if ( ADC_array[i] > var_sys.ADCZeroOffset )
        {
            auxValue = ( ADC_array[i] - var_sys.ADCZeroOffset );
        }
        else
        {
           auxValue = ( var_sys.ADCZeroOffset - ADC_array[i] );
        }
        sumE += auxValue;
...
void refreshADCZeroOffset(void)
{
    ...
    if ( var_sys.StateMotor == Stopped )
    {
        ...
        {
            ts_system.TimeStopADCZeroOffsetCalc = (1000/50);
...
unsigned long getCurrentComparator()
{
    ...
    {
        ts_system.timewaitBeforeSoftStart=(1000/50);
    ...
    if(powerControl_st.decelarationActive==NO)
    {
        ...
        if(var_sys.ActualComparatorCurrent<=100||(var_sys.ActualComparatorCurrent>(510*30)))
        {
             var_sys.ActualComparatorCurrent=(510*30);
        }
        else
        {
           ...
           var_sys.ActualComparatorCurrent=var_sys.ActualComparatorCurrent+((10*(x*x))+(-200*x)+1300);
        }
    }
    else
    {
        ...
        {
            var_sys.ActualComparatorCurrent=var_sys.ActualComparatorCurrent-(1200 +(500*(10-var_sys_NVM.decelarationSensivity)));
        }
    }
...
void ControlCurrent(void)
{
    ...
    var_sys.velocityActual = GetMotorPower();
    var_sys.ActualComparatorCurrent = getCurrentComparator();
    ...
    if(var_sys.StateMotor == Stopped)
    {
        ...
        ts_system.TimeTransitionDecelaration=(1000/50);
    }
    ...
    else if(powerControl_st.decelarationActive==NO)
    {
        ...
        if (( var_sys.ActualCurrent>var_sys.ActualComparatorCurrent ) && (ts_system.TimeChargeCondesador<=0)&&var_sys_NVM.motorSensitivity!=0 && ts_system.TimeReleCapacitor<=0)
        {
            ...
            if(ts_system.TimeCurrentWaitForAlarm<=0&&numberOfErrorCurrent>1)
            {
                ...
                ts_system.TimeCurrentAlarm= (1000/50)*10;
