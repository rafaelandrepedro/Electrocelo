...
_Bool read_eusartparser(struct package_t* package){
    ...
    for (int i=0;i<sizeof(systemVarAddresses)/sizeof(systemVarAddresses[0]);i++)
        if(systemVarAddresses[i].key==package->address){
            switch(systemVarAddresses[i].size){
                case B8:
                    package->data.data8[systemVarAddresses[i].byte]=*(uint8_t*)(systemVarAddresses[i].address);
                    break;
                case B16:
                    package->data.data16=*(uint16_t*)(systemVarAddresses[i].address);
                    break;
                case B32:
                    package->data.data16=*(((uint16_t*)systemVarAddresses[i].address)+1);
                    write_package(*package);
                    package->data.data16=*(((uint16_t*)systemVarAddresses[i].address)+0);
                    write_package(*package);
                    return 1;
                    break;
                default:
...
void write_eusartparser(struct package_t package){
    for (int i=0;i<sizeof(systemVarAddresses)/sizeof(systemVarAddresses[0]);i++){
        if(systemVarAddresses[i].key==package.address){
            switch(systemVarAddresses[i].size){
                case B8:
                    *(uint8_t*)(systemVarAddresses[i].address)=package.data.data8[systemVarAddresses[i].byte];
                    continue;
                case B16:
                    *(uint16_t*)(systemVarAddresses[i].address)=package.data.data16;
                    break;
                case B32:
                    *(uint32_t*)(systemVarAddresses[i].address)=*(uint32_t*)(systemVarAddresses[i].address)|((uint32_t)(package.data.data16&0xFFFF)<<(uint8_t)(1*16));
                    *(uint32_t*)(systemVarAddresses[i].address)=*(uint32_t*)(systemVarAddresses[i].address)&~((uint32_t)(~package.data.data16&0xFFFF)<<(uint8_t)(1*16));
                    return;
                default:
        ...
        if((systemVarAddresses[i].key==package.address-1)&&(systemVarAddresses[i].size==B32)){
            *(uint32_t*)(systemVarAddresses[i].address)=*(uint32_t*)(systemVarAddresses[i].address)|((uint32_t)(package.data.data16&0xFFFF)<<(uint8_t)(0*16));
            *(uint32_t*)(systemVarAddresses[i].address)=*(uint32_t*)(systemVarAddresses[i].address)&~((uint32_t)(~package.data.data16&0xFFFF)<<(uint8_t)(0*16));
            return;
        }
    }
...
void updateChangesToUart(void){
    ...
    for (uint8_t i=0x00;i<=sizeof(systemVarAddresses)/sizeof(systemVarAddresses[0]);i++){
        package.address=i;
        read_eusartparser(&package);
    }
    confirmpackage(&package, 1);
