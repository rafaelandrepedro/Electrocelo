...
typedef struct
{
    ...
    unsigned char decelarationOpen;
    unsigned char decelarationClose;
    unsigned char motorPower;
    unsigned char motorSensitivity;
    unsigned char walkTime;
    unsigned char autoTimeFullClose;
    unsigned char autoTimeWalkClose;
    unsigned char photoCellIsON;
    unsigned char photoCellInOpen;
    unsigned char securityBandIsON;
    unsigned char securityBandType;
    unsigned char securityBandInOpen;
    unsigned char operationMode;
    unsigned char flashLightMode;
    unsigned char programmingDistance;
    unsigned char decelarationSensivity;



    unsigned char homemPresente;
    unsigned char logicDigital;
    unsigned char softStart;
    unsigned char softstop;
    unsigned char ligthTime;
    unsigned char folow_me;
    unsigned char Stopboton;
    unsigned char electricBrake;
    unsigned char velocityDecelaration;
    unsigned char flashRGBMode;
    unsigned char reserved10;
    unsigned char Direction_motor;
    unsigned char TypeofMotor;
...
typedef struct
{
 stateMotor_enum StateMotor;
    ...
    unsigned long TimeMaxMotorIsON;
    ...
    StateEnum DistanceProgrammingActive;
...
typedef struct
{
    ...
    int timeSoftStart;
    int timeSoftStop;
    int TimeChargeCondesador;

    int timeFreqRGB;
    int timeFreqFlashligth;
    long TimeOnFlashLigth;
    int TimeWaitToNextState;

    int timeBetweenSaveCounters;
    int TimeElectricBrake;
    int TimeCurrentAlarm;

    int TimeStopedBeforeDecelaration;
    int TimeCurrentWaitForAlarm;
    int TimeInversionStart;
    int InvertionTimeClosingFromOpen;
    int TimeWaitToGetCurrent;
    int TimeLearningDecelaration;
    int TimeTransitionDecelaration;
    int timewaitBeforeSoftStart;
    int TimeShowVersion;
    int TimeStopADCZeroOffsetCalc;
    int TimeStopElectricBrake;
...
# 82 "./outputs.h"
    ...
    void SetSegmentValueMain(char d1, char d2);
    void SetSegmentValue(char d1, char d2);
    void SetSegmentValueLearn(Direction_en direction);
    void SetSegmentValueIntermitentMain(char d1, char d2,int time);
...
# 29 "./eusartpackage.h"
    ...
    _Bool valid_package(const struct package_t package);
...
char zeroCrossingWaitTime[2]={25,25};
...
volatile unsigned char DispTable[dSize] ={0x04|0x08|0x40|0x20|0X01|0x02,0x20|0X01,0x40|0x20|0x10|0x04|0x02,0x40|0x20|0X01|0x02|0x10,
                                         0x08|0x10|0x20|0X01,0x40|0x08|0x10|0X01|0x02,0x08|0x10|0X01|0x02|0x04|0x40,0x40|0x20|0X01,
                                         0x40|0x20|0X01|0x02|0x04|0x08|0x10,0x40|0x20|0X01|0x02|0x08|0x10,0x40|0x20|0X01|0x04|0x08|0x10,0x40|0x20|0X01|0x02|0x04|0x08|0x10,
                                         0x40|0x08|0x04|0x02,0x04|0x10|0x02|0X01|0x20,0x40|0x08|0x10|0x04|0x02,0x40|0x08|0x10|0x04,
                                         0x40|0x20|0x08|0x10|0x04,0x20|0X01|0x08|0x10|0x04,0x08|0x04|0x02,0x08|0x04|0x02|0X01|0x20,
                                         0x04|0x10,0x08|0x04|0x02|0x10,0x04|0x02|0X01,0x04|0X01|0x40,0x10,0x08|0x04,
                                         0x40,0x20,0X01,0x02,0x04,0x08,0x00,0x08|0x20|0x10|0X01|0x02,0x08|0x10|0X01|0x02|0x04,0x04|0x10|0x02,0x04|0x02|0x10|0X01};
...
void SetSegmentValueIntermitent(char d1, char d2,int time)
{
    ...
    if ( main_stateMachine.current_state != st_MenuConfiguration && (ts_system.timeoutMenu == 0 && button_struct.timer < (1000/50)))
    {
        dispStruct.display_grp.value_display1=d1;
        dispStruct.display_grp.value_display2=d2;
        dispStruct.intermitentFreq=time;
...
void controlMotorForce()
{
    ...
    if(powerControl_st.decelarationActive==YES)
    {
        ...
        else if(powerControl_st.actualCycle<powerControl_st.decelarationCyclesOff)
        {
    ...
    if(PORTAbits.RA3==1 && zeroCrossing==0)
    {
# 257 "outputs_MC50.c"
        if ( waitZeroPeak == 0 )
        {

            zeroCrossingWaitTime[zeroCrossingWaitTimeIdx] = (zeroCrossingLength>>1);
        ...
        if ( (difTime<=80) || (ts_system.TimeElectricBrake>0) )
        {
        ...
        if ( waitZeroPeak > zeroCrossingWaitTime[zeroCrossingWaitTimeIdx] )

        {
            ...
            if ( var_sys_NVM.electricBrake==1 && ts_system.TimeElectricBrake>0 )
            {
    ...
    else if(PORTAbits.RA3==0 && zeroCrossing!=0)
    {
   ...
   var_sys.Control50or60hz ++;
# 352 "outputs_MC50.c"
    stateMotor = VerifyElectricalBrake(var_sys.StateMotor);

    if ( var_sys_NVM.electricBrake==1 && ts_system.TimeElectricBrake>0 && ts_system.TimeStopElectricBrake<=0 )
    {

        if(stateMotor !=0 && StateTRIAC==1 && ts_system.ts_TRIACWait<=0 && powerControl_st.actualCycle==0 && ts_system.TimeElectricBrake<(((5+6)*( 100/50))-(5*( 100/50))) )
        {
           do { LATDbits.LATD7 = 1; } while(0);
           ts_system.ts_TRIACWait=-(25*200);
           var_sys.TriacON=YES;
           StateTRIAC=0;
        }
    }
    else
    {
        ...
        {
           do { LATDbits.LATD7 = 1; } while(0);
           ts_system.ts_TRIACWait=-200;
           var_sys.TriacON=YES;
           StateTRIAC=0;
        }
    }

    if (((ts_system.ts_TimeSlotTRIAC <= 20 )&& ts_system.TimeElectricBrake<0) || (stateMotor==0))
    {
...
}unsigned char GetMotorPower()
{
    ...
    if(powerControl_st.decelarationActive==YES)
    {
       temppower=(var_sys_NVM.motorPower*6);
    }
    else
    {
        fullmotorPower=(var_sys_NVM.motorPower*6);
        ...
        else if ((var_sys.velocityFactorstop > 0)){



                temppower = var_sys.velocityFactorstop;

        }
    }
    if ((var_sys.velocityFactorstop > 0)&&(var_sys.velocityFactor <= 0 )){
    defineatraso = 5;
    }
    else
    {
...
_Bool getStateOpenPosition(stateMotor_enum stateMotor)
{
    _Bool state=0;
    ...
    else if((var_sys_NVM.learningTimeToOpen>(unsigned long)(var_sys_NVM.walkTime*(1000/50))) && stateMotor==InOpenWalk)
    {
        if(var_sys_NVM.walkTime<(var_sys_NVM.decelarationOpen/2))
        {
            state= 1;
        }
        else if(var_sys.PositionActual>((signed long)(var_sys_NVM.walkTime*(1000/50)) - (1000/50)*2))
        {
            state= 1;
        }
        else
    ...
    {
         if((var_sys_NVM.learningTimeToOpen)<(((unsigned long)var_sys_NVM.decelarationOpen*( 500/50))))
        {
            state= 1;
        }
...
_Bool getStateClosePosition(stateMotor_enum stateMotor)
{
    _Bool state=0;
    if(var_sys.PositionIsLost==YES)
    {
...
void controlVelocity(void)
{
    ...
    if(var_sys.LearningIsEnabled==YES && var_sys.LearningDecelaration==YES)
    {
        if(powerControl_st.decelarationActive==NO)
        {
            ...
            ts_system.TimeCurrentWaitForAlarm=(1000/50);
        }
    ...
    else if(var_sys.StateMotor==InClose && ((((signed long)var_sys.PositionActual)<(getTimeDecelaration(InClose)) || var_sys.PositionIsLost==YES) && ((var_sys_NVM.decelarationClose != 0 && var_sys.AutoInversionActiveStop == NO) || ((var_sys.velocityFactorstop <= 0) && var_sys_NVM.softstop >0))))
    {
            ...
            if ((powerControl_st.decelarationActive==NO) && (var_sys.AutoInversionActiveStop==NO ))
                ...
                {
                ts_system.TimeStopedBeforeDecelaration=( 100/50);
                var_sys.AutoInversionActiveStop = NO;
                ControlDecelarationFactor();
                powerControl_st.decelarationActive=YES;
                var_sys.velocityFactor=0;
    ...
    else if((getStateOpenPosition(var_sys.StateMotor)==1)&& ((var_sys_NVM.decelarationOpen != 0 && var_sys.AutoInversionActiveStop == NO) || (var_sys.velocityFactorstop <= 0) && var_sys_NVM.softstop > 0))
    {
...
signed long getTimeDecelaration(stateMotor_enum state)
{
    ...
    if(state==InClose)
    {
        ...
        {return (signed long)((signed long)var_sys_NVM.decelarationClose*( 500/50));}
        else{return (signed long)((signed long)var_sys_NVM.decelarationClose*( 50/50)*4);}
   }
    else
    {
        if (var_sys_NVM.TypeofMotor != 2)
        {return (signed long)((signed long)var_sys_NVM.learningTimeToOpen-(((signed long)var_sys_NVM.decelarationOpen*( 500/50))));}
        else{return (signed long)((signed long)var_sys_NVM.learningTimeToOpen-(((signed long)var_sys_NVM.decelarationOpen*( 50/50)*4)));}
    }
}
...
stateMotor_enum VerifyElectricalBrake(stateMotor_enum actualState)
{
    ...
    if(var_sys_NVM.electricBrake==1 && actualState==Stopped && ts_system.TimeStopElectricBrake<=0)
    {
        if(actualState!=brakeElectric_st.lastState && brakeElectric_st.ElectricBrakeIsActive==NO )
        {
            ...
            ts_system.TimeElectricBrake=((5+6)*( 100/50));
...
void ControlMotor (void)
{
    ...
    if(stateMotor == Stopped )
    {
        var_sys.InStoping = YES;
    ...
    else if ( (var_sys_NVM.electricBrake==1) && (brakeElectric_st.ElectricBrakeIsActive==YES) && (var_sys.StateMotor==Stopped) && (stateMotor!=Stopped) && (ts_system.TimeElectricBrake>0) && (ts_system.TimeStopElectricBrake<=0) )
    {


        if ( ts_system.TimeElectricBrake<(((5+6)*( 100/50))-(2*( 100/50))) )
        {
    ...
    else if(((stateMotor == InOpenFull) || (stateMotor == InOpenWalk)) && ((var_sys.FimCurso_OpenIsEnabled == NO)))
    {
        if((PORTEbits.RE1==0 && var_sys_NVM.Direction_motor == 0)||(PORTEbits.RE0==0 && var_sys_NVM.Direction_motor == 1))
        {
            if((PORTEbits.RE0==0 && var_sys_NVM.Direction_motor == 0)||(PORTEbits.RE1==0 && var_sys_NVM.Direction_motor == 1))
            {
                var_sys.InStoping = NO;
                ts_system.ts_TRIACWait=200;
                ts_system.TimeChargeCondesador=(1000/50);
                if (var_sys.FimCurso_CloseIsEnabled == YES|| var_sys.Statedoorcontrol == st_Closed ){
                var_sys.TimeMaxMotorIsON=((unsigned int)var_sys_NVM.velocityDecelaration+((unsigned int)var_sys_NVM.decelarationOpen)*( 500/50))+var_sys_NVM.learningTimeToOpen + (1000/50)*4;
            ...
            if (var_sys_NVM.Direction_motor == 0){do { LATEbits.LATE0 = 1; } while(0);}
            else{do { LATEbits.LATE1 = 1; } while(0);}
            var_sys.Activatecounter = open ;

        }

           if (var_sys_NVM.Direction_motor == 0){do { LATEbits.LATE1 = 0; } while(0);}
           else{do { LATEbits.LATE0 = 0; } while(0);}

    }

    else if((stateMotor == InClose) && (var_sys.FimCurso_CloseIsEnabled == NO))
    {
        if((PORTEbits.RE0==0 && var_sys_NVM.Direction_motor == 0)||(PORTEbits.RE1==0 && var_sys_NVM.Direction_motor == 1))
        {
            if((PORTEbits.RE1==0 && var_sys_NVM.Direction_motor == 0)||(PORTEbits.RE0==0 && var_sys_NVM.Direction_motor == 1))
            {
                ...
                var_sys.TimeMaxMotorIsON=((unsigned int)var_sys_NVM.velocityDecelaration+(unsigned int)var_sys_NVM.decelarationClose*( 500/50))+var_sys_NVM.learningTimeToClose + (1000/50)*4;

                }
                var_sys.Statedoorcontrol = st_Closing;

            }
            if (var_sys_NVM.Direction_motor == 0){do { LATEbits.LATE1 = 1; } while(0);}
            else{do { LATEbits.LATE0 = 1; } while(0);}
            var_sys.Activatecounter = close ;

        }
           if (var_sys_NVM.Direction_motor == 0){do { LATEbits.LATE0 = 0; } while(0);}
           else{do { LATEbits.LATE1 = 0; } while(0);}
    }
}
...
void ControlLights(void)
{
    ...
    if (((var_sys.preflashingcontrol == YES || controlGate_stateMachine.current_state==st_Opened) && (ts_system.timeWaitToClose >0) && (ts_system.timeWaitToClose <= 3 * (1000/50))) && var_sys_NVM.flashRGBMode == rgb_pre){
        setRGB_ON(BLUE,( 200/50));

    }


    else if(var_sys.StateMotor == InClose)
    {
        var_sys.ControlReleCapacitorOpen = 0;


        if (var_sys.ControlReleCapacitorClose==0 && var_sys_NVM.flashRGBMode==rgb_capacitor)
        {
         ts_system.TimeReleCapacitor = (1000/50)*2;
            setRGB_ON(YELLOW,0);
    ...
    else if(var_sys.StateMotor == Stopped && controlGate_stateMachine.current_state==st_Closed)
    {
        ...
        {
            if (var_sys_NVM.TypeofMotor == br || var_sys_NVM.TypeofMotor == SE)setRGB_ON(YELLOW,0);
            else setRGB_ON(YELLOW,(1000/50)*2);
        }
    ...
    else
    {
        ...
        var_sys.ControlReleCapacitorClose=0;
        if (var_sys.ControlReleCapacitorOpen==0 && var_sys_NVM.flashRGBMode==rgb_capacitor)
        {
         ts_system.TimeReleCapacitor = (1000/50)*2;
