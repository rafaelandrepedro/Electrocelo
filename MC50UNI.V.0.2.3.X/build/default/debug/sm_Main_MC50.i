...
void sm_execute_main( sm_t *psm ) {
    ...
    switch((sm_state_Main_t) psm->current_state)
    {
        case st_standBy:
        {
            ...
            else if(button_struct.current==0x00 && var_sys.FimCurso_CloseIsEnabled==NO)
            {
                var_sys.StartFromButton=YES;
                var_sys.StateMotor=InClose;
            }
            else if(button_struct.current==0x17 && var_sys.FimCurso_OpenIsEnabled==NO)
            {
                var_sys.StartFromButton=YES;
            ...
            else if((rfCMD.currentType==TypeProgrammingModeFull||rfCMD.currentType==TypeProgrammingModeWalk ) && rfCMD.timer>=((1000/50)*5))
            {
                load_New_Menu(S_Menu);
                sm_send_event(&main_stateMachine, ev_goto_MenuConfig);
        ...
        case st_MotorON:
        {
            ...
            if( ts_system.timeSinalizationDigit<=0 && (button_struct.current!=0x27 && button_struct.current!=0x07) )
            {
                if(var_sys.StateMotor==InClose )
                {
                   ...
                   SetSegmentValueIntermitentMain(dC,dL,( 500/50));
                ...
                else if(var_sys.StateMotor==InOpenWalk || var_sys.StateMotor==InOpenFull)
                {
                    ...
                    SetSegmentValueIntermitentMain(d0,dP,( 500/50));
                    ts_system.timeSinalizationDigit=( 200/50);
                ...
                else if(var_sys.StateMotor==Stopped && controlGate_stateMachine.current_state==st_Opened)
                {
                    ...
                    (
                        ( var_sys_NVM.operationMode!=ModoPassoPasso && var_sys.AutoCloseActive!=0 && var_sys.LastState==InOpenWalk ) ||
