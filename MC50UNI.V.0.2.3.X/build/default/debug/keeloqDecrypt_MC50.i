...
# 2 "<built-in>" 2
# 1 "keeloqDecrypt_MC50.c" 2
# 26 "keeloqDecrypt_MC50.c"
...
uint8_t DKEY[8];
volatile classic_nvm_t listSyncValues[25];
...
void SecureKeyGen(uint8_t* seed) {
    ...
    LoadManufCode();
    Decrypt(HOPtemp);
    memcpy(SKEYtemp, HOPtemp, 4);


    memcpy(HOPtemp, &Buffer[4], 4);
    LoadManufCode();
    Decrypt(HOPtemp);
    memcpy(&SKEYtemp[4], HOPtemp, 4);
    memcpy(DKEY, SKEYtemp, 8);
...
TypeCMD keeloqDecryptPacket (char* data,classic_encoder_t* classic_data)
{
    ...
    {
        if(validateRemoteSerialNumber(classic_data->serialnumber,YES,&classic_data->positionMem)==NoCMD)
        {
...
static void keeloq_classic_decrypt_packet(classic_encoder_t* data) {
    ...
    NormalKeyGen(&data->serialnumber);

    Decrypt((uint8_t*) & data->sync);
...
TypeCMD verifyProgramingMode(classic_encoder_t* classic_data)
{
    if (classic_data->fcode1==0x0A)
    {
        return TypeProgrammingModeFull;
    }
    else if (classic_data->fcode1==0x05)
    {
