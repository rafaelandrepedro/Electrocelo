...
# 12 "sm_ControlGate_MC50.c" 2

# 1 "./sm_ControlLearning.h" 1
# 18 "./sm_ControlLearning.h"
typedef struct {
    long timeClose;
    long timeOpen;
...
# 14 "sm_ControlGate_MC50.c" 2

# 1 "./sm_MenuConfiguration.h" 1
# 16 "./sm_MenuConfiguration.h"
typedef enum {
 st_MainMenu,
 st_EditParameter,
 st_SaveParameter,
...
void WaitPreFlash(void)
{
    ...
    if ((var_sys_NVM.flashRGBMode == rgb_pre) && (var_sys.preflashingcontrol == YES) && (ts_system.timewaitFlashRGB <=0) && (aux_i == 0))
        {
            ts_system.timewaitFlashRGB = (1000/50) *3;
            aux_i=1;
...
void OpenWalkOrder(void) {
    int diffTime = ((int) var_sys_NVM.walkTime * (1000/50)) - var_sys.PositionActual ;
    ...
    if (diffTime > 0) {
        ...
        if (var_sys.Statedoorcontrol == st_Closed){
           ts_system.timeOpenWalk = diffTime;
            ...
            ts_system.timeOpenWalk = diffTime + (1000/50);
        }
...
void InitWalkAutoTime(void) {
    ...
    ts_system.timeWaitToClose = ((int) var_sys_NVM.autoTimeWalkClose * (1000/50));
...
void InitFullAutoTime(void) {
    ...
    if (var_sys_NVM.homemPresente == 0) {
        ts_system.timeWaitToClose = ((int) var_sys_NVM.autoTimeFullClose * (1000/50));
...
void VerifyFollowMe(StateEnum securityClose) {
    ...
    if (var_sys_NVM.folow_me != 0 && var_sys_NVM.photoCellInOpen == 0)
    {
        if (securityClose == YES && var_sys.StateFollowIsOn == NO) {
            var_sys.StateFollowIsOn = YES;
        }
    }
}
...
void sm_execute_ControlGate(sm_t *psm) {
    ...
    switch ((sm_state_ControlGate_t) psm->current_state) {
        case st_Idle:
        {
            ...
            ts_system.TimeStopADCZeroOffsetCalc = (5 * (1000/50));


            var_sys.PositionIsLost = YES;
            if (var_sys.WorkTimeMaxAlarmState == EmAlarme || var_sys.NumberOffErrors > 3)
            {
            ...
            else
            {
                    ...
                    ts_system.timeSinalizationDigit = (1000/50) * 2;
                    SetSegmentValueIntermitent(dF, dC, ( 200/50));
...
void actionZone_Closed(void) {
    ...
    if (main_stateMachine.current_state != st_MenuConfiguration) {
        ...
        StateEnum securityOpening = GetSecurityStateInOpening();
...
void actionZone_Opening(void) {
    StateEnum securityClosing = GetSecurityStateInClosing();
    StateEnum securityOpening = GetSecurityStateInOpening();
    TypeCMD typeOpenOrder = GetOpenOrder();
    ...
    if (var_sys.NumberOffErrors > 3) {

        var_sys.StateMotor = Stopped;
        sm_send_event(&controlGate_stateMachine, ev_ErrorGate);
    } else if (var_sys.StateMotor == InOpenWalk && ts_system.timeOpenWalk <= 0)
    {
        ...
        StopOrder();
        InitWalkAutoTime();
        var_sys.passoAPassoAutoClose = YES;
        Abertura_pedonal = 1;

    } else if ((var_sys.FimCurso_OpenIsEnabled == YES)||(var_sys.TimeMaxMotorIsON<=0) && (var_sys.LearningIsEnabled==NO) && (var_sys.PositionIsLost==NO) && (var_sys.Statedoorcontrol == st_Opening))
    {
        ...
        var_sys.NumberOffErrors = 0;
        ts_system.timeSinalizationDigit = (1000/50) * 2;
        SetSegmentValueIntermitent(dF, d0, ( 200/50));
        StopOrder();
        var_sys.passoAPassoAutoClose = YES;
        ...
        ts_system.TimeWaitToNextState = ( 500/50);
        ts_system.TimeStopElectricBrake = ((1000/50)+( 500/50));
        ...
        if (var_sys.InvertionCurrentClosing == YES) {
            ...
            ts_system.TimeInversionStart = (1000/50);
            ts_system.InvertionTimeClosingFromOpen = (1000/50) * 2;
...
void actionZone_Opened(void) {
    ...
    if ((PORTBbits.RB1==0) && (var_sys_NVM.autoTimeFullClose != 0)&&(var_sys.AutoCloseActive != 0) && (button_struct.current != 0x35)&& (var_sys_NVM.operationMode == ModoCondominio)){
      typeOpenOrder = NoCMD;
    }
     ...
     if (typeOpenOrder != NoCMD)
     {
        ...
        ts_system.timeSinalizationDigit = (1000/50) * 3;

       if ( var_sys_NVM.operationMode == ModoCondominio && var_sys_NVM.autoTimeFullClose == 0 && (typeOpenOrder == TypeFull|| typeOpenOrder == TypeFullClose ))
        {
    ...
    else if (PORTBbits.RB1==0 && var_sys_NVM.autoTimeFullClose != 0){
        ts_system.timeWaitToClose = ((int) var_sys_NVM.autoTimeFullClose * (1000/50));
        ...
        ts_system.timeWaitToClose = var_sys_NVM.folow_me * (1000/50);

    }else if (ts_system.timeWaitToClose <= 0 && var_sys_NVM.operationMode == ModoPassoPasso && var_sys.passoAPassoAutoClose == YES && var_sys.AutoCloseActive != 0) {
    ...
    } else if ((var_sys.DigitSinalizedTemp == YES) && (securityClosing == NO))
    {
...
void actionZone_Closing(void) {
    ...
    if (var_sys.NumberOffErrors > 3) {
        ...
        if (var_sys_NVM.flashLightMode == 2) {
            ts_system.TimeOnFlashLigth = (long) ((long) var_sys_NVM.ligthTime * ((1000/50)*60));
        }
        else {
        ...
        if ( var_sys_NVM.operationMode == ModoPassoPasso )
        {
            ...
            if (typeOpenOrder == TypeWalk && var_sys_NVM.operationMode != ModoCondominio)
            {
                InitWalkAutoTime();
                var_sys.showAP = YES;
            }
...
void InitSoftStart(void) {
    if (var_sys_NVM.softStart != 0 && var_sys.AutoInversionActive == NO) {
        ...
        if ( var_sys.velocityFactor > (81) )
        {
            var_sys.velocityFactor = (81);
        }
        else
        {
...
void setPositionInvertionInClosing() {
    ...
    if(var_sys_NVM.TypeofMotor != 2){
       ...
       var_sys.PositionActual += ( 100/50);

    }
    else
    {
       var_sys.PositionActual -= ( 50/50);
    }
    ...
    var_sys.TimeMaxMotorIsON = ((signed long)var_sys_NVM.learningTimeToOpen - var_sys.PositionActual) + (var_sys_NVM.decelarationOpen*( 500/50))+ (1000/50)*10;
...
void setPositionInvertionInOpening() {
    ...
    var_sys.TimeMaxMotorIsON = var_sys.PositionActual + var_sys_NVM.decelarationClose*( 500/50) + (1000/50)*10;
