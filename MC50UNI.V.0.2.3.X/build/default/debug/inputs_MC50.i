...
_Bool DecCHK(classic_encoder_t* data);
...
unsigned char BufferRF[9];
unsigned char BufferRF_12BIT[2];

static unsigned char RFstate;
static signed int RFcount;
static unsigned char Bptr;
static unsigned char BitCount;

volatile char RFFull;
volatile char RFFull_12BIT;
volatile char RFBit;
    ...
    switch (RFstate)
    {
        ...
        case 1:
            if (RFBit) {
                ...
                if ((RFcount < 20) || (RFcount >= 45)) {
                    if((RFcount < 80) || RFcount>=100)
                    {
                        RFstate = 0;
                    }
                    else
...
void readRFButtons() {
    ...
    if(RFFull)
    {
         ...
         if(RFFull_12BIT==0)
         {
            keyType = keeloqDecryptPacket(BufferRF,&classic_data);
         }
         else
         {
            keyType = DecryptPacket_12BIT(BufferRF,&classic_data);
         }
...
# 270 "inputs_MC50.c"
        ...
        ts_system.TimeLastCommand=( 500/50);
...
TypeCMD GetOpenOrder(void){
   ...
   if (var_sys.LearningIsEnabled == NO){
       ...
       if(ts_system.TimeCurrentAlarm>0)
        ...
        {
            SetSegmentValueIntermitentMain(dL,d1,(1000/50));
        ...
        {
            if(button_struct.current==0x35)
            {
                ts_system.timeSinalizationDigit=( 500/50);
                SetSegmentValueIntermitentMain(dL,d0,( 200/50));
                if (var_sys_NVM.homemPresente == 2){
                   button_struct.processed=1;
                }
                return TypeFullOpen;
            }
            else if(button_struct.current==0x33)
            {
                ts_system.timeSinalizationDigit=( 500/50);
                SetSegmentValueIntermitentMain(dL,d5,( 200/50));
        ...
        else if(button_struct.current==0x35)
        {
        ...
        else if(button_struct.current==0X36)
        {
            ts_system.timeSinalizationDigit=(1000/50)*2;
            SetSegmentValueIntermitentMain(dP,dU,( 200/50));
            {
                button_struct.processed=1;
                return TypeFull;
            }
        ...
        else
        {
            ...
            {
                SetSegmentValueIntermitentMain(rfCMD.currentPosMem/10, rfCMD.currentPosMem-((rfCMD.currentPosMem/10)*10),( 200/50));
                ts_system.timeSinalizationDigit=(1000/50)*2;
                rfCMD.processed=1;
            }
    ...
    }else
    {
       if(rfCMD.currentType!=NoCMD ){
                  SetSegmentValueIntermitentMain(rfCMD.currentPosMem/10, rfCMD.currentPosMem-((rfCMD.currentPosMem/10)*10),( 200/50));
                ts_system.timeSinalizationDigit=(1000/50)*1;
                rfCMD.processed=1;
       }
       return rfCMD.currentType;
...
void readbutton(void){
 ...
 if ( key2 == key1 )
 {
    ...
    {
        button_struct.current = 0x37;
    }

    if ( button_struct.current == 0x37 || ( button_struct.current != button_struct.old && button_struct.current == 0x00))
 {
  button_struct.processed=0;
        button_struct.timer=0;
 }

 if ( button_struct.current != 0x37 )
 {
...
void readInputs(void){
    ...
    if(var_sys_NVM.Direction_motor == 0){fimdecurso.FimdeCursoCloseAUX=PORTAbits.RA1;}
    else {fimdecurso.FimdeCursoCloseAUX=PORTAbits.RA2;}
    ...
    {
           fimdecurso.FimdeCursoCloseTimer++;
    }

    else
    {
            fimdecurso.FimdeCursoCloseOLD = fimdecurso.FimdeCursoCloseAUX;
            fimdecurso.FimdeCursoCloseTimer=0;
    }


    if (fimdecurso.FimdeCursoCloseTimer>=( 50/50))
    {
    ...
    if(var_sys_NVM.Direction_motor == 0){fimdecurso.FimdeCursoOpenAUX=PORTAbits.RA2;}
    else {fimdecurso.FimdeCursoOpenAUX=PORTAbits.RA1;}
    ...
    {
           fimdecurso.FimdeCursoOpenTimer++;
    }

    else
    {
            fimdecurso.FimdeCursoOpenOLD = fimdecurso.FimdeCursoOpenAUX;
            fimdecurso.FimdeCursoOpenTimer=0;
    }


    if (fimdecurso.FimdeCursoOpenTimer>=( 50/50))
    {
    ...
    security.PhotoCellAUX=PORTAbits.RA4;
    ...
    {
           security.PhotoCellTimer++;
    }

    else
    {
            security.PhotoCellOLD = security.PhotoCellAUX;
            security.PhotoCellTimer=0;
    }


    if (security.PhotoCellTimer>=( 50/50))
    {
        var_sys.photoCellIsObstructed=security.PhotoCellAUX;
    }


    security.SecurityBarAUX=PORTAbits.RA5;



    if (security.SecurityBarAUX==security.SecurityBarOLD)
    {
           security.SecurityBarTimer++;
    }

    else
    {
            security.SecurityBarOLD = security.SecurityBarAUX;
            security.SecurityBarTimer=0;
    }


    if (security.SecurityBarTimer>=( 50/50))
    {
        security.SecurityBarSTATE=security.SecurityBarAUX;
    }

    security.SecurityBar8K2AUX=PORTAbits.RA6;



    if (security.SecurityBar8K2AUX==security.SecurityBar8K2OLD)
    {
           security.SecurityBar8K2Timer++;
    }

    else
    {
            security.SecurityBar8K2OLD = security.SecurityBar8K2AUX;
            security.SecurityBar8K2Timer=0;
    }


    if (security.SecurityBar8K2Timer>=( 50/50))
    {
...
void ControlInputs(void){
    ...
    if (( var_sys.StateMotor==InClose || (((PORTEbits.RE1 == 1)&& var_sys_NVM.Direction_motor == 0)||((PORTEbits.RE0 == 1)&&var_sys_NVM.Direction_motor == 1))) && (var_sys.FimCurso_CloseIsEnabled == YES))
    {
        var_sys.StateMotor = Stopped;

    }
    if((var_sys.StateMotor==InOpenFull || var_sys.StateMotor==InOpenWalk || (((PORTEbits.RE0 == 1) &&var_sys_NVM.Direction_motor == 0)||((PORTEbits.RE1 == 1) && var_sys_NVM.Direction_motor == 1))) && (var_sys.FimCurso_OpenIsEnabled == YES))
    {
        var_sys.StateMotor = Stopped;
...
StateEnum GetSecurityStateInOpening()
{
    ...
    if((var_sys_NVM.securityBandIsON == 1) && (var_sys.SecurityBarIsObstructed == YES)&&(var_sys_NVM.securityBandInOpen == 1)&& (var_sys_NVM.Stopboton == 0))
    {
        ...
        ts_system.timeSinalizationDigit=( 200/50);
        SetSegmentValueIntermitent(dL,dA,(1000/50));
        return YES;
    }

    else if((var_sys_NVM.Stopboton == 1) && (var_sys.SecurityBarIsObstructed == YES))
    {

        ts_system.timeSinalizationDigit=( 200/50);
        SetSegmentValueIntermitent(d5,dt,(1000/50));
        return ST;
    }
    ...
    else if( (var_sys_NVM.photoCellIsON==1) && (var_sys.photoCellIsObstructed==YES)&&(var_sys_NVM.photoCellInOpen==1) )
    {
        ...
        ts_system.timeSinalizationDigit=( 200/50);
        SetSegmentValueIntermitent(dL,dE,(1000/50));
        return YES;
    }
    else
    {
