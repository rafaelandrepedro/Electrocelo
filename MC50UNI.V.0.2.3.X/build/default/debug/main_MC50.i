...
void main(void)
{
    ...
    ts_system.Debug_TS=(1000/50);

    var_sys.ADCZeroOffset = 510;
    ts_system.TimeStopADCZeroOffsetCalc = (5*(1000/50));
    ts_system.TimeStopElectricBrake = 0;
        ...
        if ( ts_system.ts_TimeSlotMain<=0 )
        {
            ...
            else
            {
               ...
               lastKey=getKeyPressed();
                ...
                sm_execute_main(&main_stateMachine);
                sm_execute_ControlGate(&controlGate_stateMachine);
...
void controlTimeslots(void)
{
 decrementTimeslot((int*)&ts_system.Debug_TS);
    ...
    if ( ( var_sys_NVM.electricBrake==0 ) || ( ts_system.TimeElectricBrake<=0 ) )
    {
    ...
    decrementTimeslot((int*)&ts_system.TimeStopElectricBrake);
...
void ShowVersion(void)
{
    if(ts_system.TimeShowVersion<=0)
    {
            ...
            {
                SetSegmentValue((1/10)%10,(1/1)%10);
            }
            else if(var_sys.StateVersion==1)
            {
                SetSegmentValue(dr, (180/100)%10);
            }
            else if(var_sys.StateVersion==2)
            {
                SetSegmentValue((180/10)%10,(180/1)%10);
            }
             else if (var_sys.StateVersion==3)
            {
               SetSegmentValue(dr, d2);
        ...
        ts_system.TimeShowVersion=(1000/50);
    }
}
